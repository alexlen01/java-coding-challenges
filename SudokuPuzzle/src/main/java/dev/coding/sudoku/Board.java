...
/**
 * This class represents Sudoku board. It contains methods
 * to get the particular areas of the board and to retrieve/set values
 */
final class Board {
	 ...
	 */
	Board(int[][] init) {
		// Check on the size of the input data and throw an IllegalArgumentException
		if (init == null || init.length != GRID_SIZE) {
			throw new IllegalArgumentException(ERROR_MSG_SIZE);
		}
		// Create a deep copy to ensure isolation/security
		board = Arrays.stream(init).toArray(int[][]::new);
	}
	...
	/**
	 * Appends the value, or a 0 if empty
	 */
	...
	private void appendLine(StringBuilder buffer) {
		...
		// Thread safe because the Sudoku class create one new Board for every
